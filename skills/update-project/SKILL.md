...
| 1.3.0 → 1.4.0 | No mandatory project-file migration. The RFN epic (batch-refine / cluster-HITL) is additive + opt-in: a new `REFINING` state and `/capiva:refine` skill, the `context-answerer` agent, the reinforcement checks, validators, and ADRs 0017/0018 — all engine-side. OPTIONAL: `.board/harness-config.md` MAY add `- **Context Answerer**: off` (absent = off; scoped to `/capiva:refine`). `.board/decisions.md` and the review packet are created on demand by the refine/auto workflows (gitignored `.state`/`.board`). The batch-refine workflow ships EXPERIMENTAL/opt-in. Re-stamp only. |
| 1.4.0 → 1.5.0 | No mandatory project-file migration. Additive/opt-in: the two-sprint cycle (RFN-013 — `/capiva:auto` pre-flight gate + refine→auto handoff), the execution docs-generation step (RFN-012), refine clarity (RFN-011), and invocation-syntax + update-command doc fixes (DIST-003/004) — all engine-side. NEW tracked scaffold `docs/features/` (project-facing feature docs): `/capiva:init` now creates it, and the auto docs-step auto-creates it on demand — existing projects need no action (OPTIONAL: `mkdir docs/features` + seed an `INDEX.md` from the template if you want it pre-created). OPTIONAL: `.board/harness-config.md` MAY add `- **Auto Preflight**: off` (absent = on; skips the interactive pre-flight for scheduled runs). Re-stamp only. |
