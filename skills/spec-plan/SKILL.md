...
lane at TRIAGE (see /sprint lane selection and ADR-0010). One pass, one human
...
3. Verify Lane = fast
4. If either check fails → **STOP**: "⛔ Phase guard failed. Current: [phase]/[lane]. Required: SPEC_PLAN/fast. Run /sprint to check state."
5. If both pass → proceed

## Fast-Lane Qualification Re-Check (MANDATORY)

/sprint qualified this task at TRIAGE, but you see more than /sprint did. Re-verify
...
3. State: "⚠️ Task does not qualify for the fast lane ([reason]). Rerouting to the full pipeline at /grill-spec."
...
- Read `docs/CONTEXT.md` for domain terms; `docs/adr/` for prior rulings
...
as /grill-spec). If the codebase answers a question, state the finding instead of
...
lane (see /grill-spec Step 6b and ADR-0009): one entry per AC, `status: "pending"`,
...
required in the fast lane — note "fast lane: tech context inline" so /implement's
guard knows).
...
🧑 ONE gate covers both: approve spec + plan to proceed to /implement,
   or request changes, or say "full pipeline" to reroute to /grill-spec.
```
...
- **TDD unchanged.** Every PLAN.md task has a test-first section; /implement enforces it.
