...
Enforcement scope: Edit/Write/NotebookEdit to source paths denied unless Phase = IMPLEMENT (test paths also allowed in TEST_VERIFY); `gh pr create` denied unless Phase = FINISH with Quality Gate PASS/ACCEPTED_SOFT_FAIL. Pipeline artifacts (.board/, docs/, .claude/, templates/, PLAN.md, root *.md) are writable in every phase. The guard fails open with a stderr warning when sprint-state.md is missing or unparseable, and honors a `CAPIVA_PHASE_GUARD=off` escape hatch — enforcement must never brick a project.
