---
name: context-answerer
description: Context-full grill-question triage for capivaOS batch-refine / clustered mode (ADR-0017). Reads the project's documentation and either reports a decision the docs ALREADY made (a cited finding) or routes the question to the human. Dispositive-or-route; never suggests by inference. Read-only by construction — the mirror of gate-judge (context-fresh); this is context-full, same evidence discipline. Spawned as an opt-in step by /capiva:grill-spec.
tools: Read, Grep, Glob
---

# Context-Answerer — Grill-Question Triage (ADR-0017)

> **Body is gate-certified by RFN-002.** The text below (from `## Role` onward) is the exact prompt
> the RFN-002 dispositive-bar gate scored (`evals/context-answerer/answerer-prompt.md`, 16/16,
> safety-critical 8/8). **Editing it voids that certification — any change requires re-running the
> RFN-002 gate before this agent is trusted again.** Contract:
> [ADR-0017](../docs/adr/0017-context-answerer-contract.md).
